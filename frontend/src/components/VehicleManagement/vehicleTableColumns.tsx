...
const GenerateColumns = (
  ...
  return [
    {
      title: (
        <span style={{ padding: "0 14px", display: "inline-block" }}>VIN</span>
      ),
      dataIndex: "vin",
      key: "vin",
      width: "21%",
    ...
    },
    {
      ...
      sorter: (a: Record<string, unknown>, b: Record<string, unknown>) => {
        const aVehicle = a as Vehicle;
        const bVehicle = b as Vehicle;
        return (aVehicle.license_plate || "").localeCompare(
          bVehicle.license_plate || ""
        );
      },
      sortOrder:
        ...
        return (
          <Space size="small">
            <Button
              type="text"
              icon={<EditOutlined />}
              onClick={() => onOpenModal(vehicle, true)}
            />
            <Popconfirm
