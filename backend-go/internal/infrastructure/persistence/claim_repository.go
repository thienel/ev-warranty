...
func (c *claimRepository) FindByID(ctx context.Context, id uuid.UUID) (*entity.Claim, error) {
	...
	if err := c.db.WithContext(ctx).Where("id = ?", id).First(&claim).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, apperror.ErrNotFoundError.WithMessage("Claim not found").WithError(err)
		}
...
func (c *claimRepository) CountPendingByTechnician(ctx context.Context, id uuid.UUID) (int64, error) {
	var count int64

	err := c.db.WithContext(ctx).
		Model(&entity.Claim{}).
		Where("technician_id = ? AND status NOT IN ?", id, []string{entity.ClaimStatusCompleted}).
		Count(&count).Error
